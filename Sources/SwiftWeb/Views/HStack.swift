...
public struct HStack<Content>: Stack, GrowingAxesModifying where Content: View {
    ...
    public var modifiedGrowingLayoutAxes: Set<GrowingLayoutAxis> {
        // `.undetermined` means that there is a spacer among the subviews which is not contained in
        // another stack. This means that this horizontal stack view can grow among its primary
        // axis.
